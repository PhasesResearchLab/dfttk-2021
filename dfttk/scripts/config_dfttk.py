...
def add_path_var(force_override=True, **kwarg):
    ...
    cshrc = homepath + "/.cshrc"
    if os.path.exists (cshrc):
        shutil.copyfile(cshrc, cshrc + ".dfttk.bak")  #backup the .bashrc file
    tcshrc = homepath + "/.tcshrc"
    if os.path.exists (tcshrc):
        shutil.copyfile(tcshrc, tcshrc + ".dfttk.bak")  #backup the .bashrc file
...
def config_atomate(path_to_store_config=".", config_folder="config", queue_script="vaspjob.pbs",
    queue_type="pbs", vasp_cmd_flag="vasp_std", machine="aci", machines=None, nodes=1, ppn=16):
    """
